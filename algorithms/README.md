...
### 3️⃣ Функции потерь
- Для регрессии (MSE, MAE, Huber)
- Для классификации (Cross-Entropy)
- Для специальных задач (Focal Loss, Contrastive)
...
### 5️⃣ Метрики качества
- Для регрессии (MSE, R², MAE)
- Для классификации (Accuracy, Precision, Recall)
- Для специальных задач (BLEU, ROUGE)
- Бизнес-метрики